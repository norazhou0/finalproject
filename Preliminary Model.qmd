...
flight_2023_filtered <- flight_2023 |>
  filter(origin == top10_airport_23$origin)
...
flight_2023_filtered <- flight_2023_filtered |>
  filter(op_unique_carrier == top5_carrier_23$op_unique_carrier)

top10_airport_24 <- flight_2024 |>
  group_by(origin) |>
  summarize(count = n()) |>
  arrange(desc(count)) |>
    slice_head(n = 10)
flight_2024_filtered <- flight_2024 |>
  filter(origin == top10_airport_24$origin)

top5_carrier_24 <- flight_2023 |>
  group_by(op_unique_carrier) |>
  summarize(count = n()) |>
  arrange(desc(count)) |>
    slice_head(n = 5)
flight_2024_filtered <- flight_2024_filtered |>
  filter(op_unique_carrier == top5_carrier_24$op_unique_carrier)
...
```{r}
fit.glm <- glm(dep_del15 ~ origin + op_unique_carrier + distance + actual_elapsed_time, family = "binomial", flight_2023_filtered)
